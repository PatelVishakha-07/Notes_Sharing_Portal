...
<form method="GET" action="{{ url('admin_dashboard') }}" class="mt-3 mb-3">
    ...
    <div class="search-pill">
        ...
        <input type="text" name="search" class="search-input" placeholder="enter title or user name" value="{{ request('search') }}">
    ...
    <div class="card-body p-0">
        ...
        <div class="table-responsive">
            ...
            <table class="table admin-table clean-table mb-0">
                        ...
                        <td>
                            <div class="d-flex gap-1 flex-wrap">
                                ...
                                @foreach ($note->filePath ?? [] as $fp)
                                    <a href="{{ url('view-file/'.$fp->file_path) }}" target="_blank" class="browse-btn" style="font-size:10px; padding:3px 7px;">
                                        📄
                                    </a>
                                @endforeach                                
                                ...
                                @if($note->youtubeLink && $note->youtubeLink->count())
                                    @foreach ($note->youtubeLink as $yt)
                                        ...
                                        @if(!empty($yt->youtube_link))
                                            ...
                                            class="btn btn-sm py-0 px-2" style="background: #ef4444; color: white; font-size: 10px; line-height: 2;">
                                                ▶
                                            </a>
                                        @endif
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center text-muted py-4"> No notes found </td>
                    </tr>
                @endforelse
